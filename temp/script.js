$(document).ready(function() {
    ...
    $('#timer').countdown({until: new Date(2015, 07, 07, 8, 20, 0), timezone: +10,
    layout: 
    ...
    $('.play_button').click(function() {
        //stop the video
        if (!window.isMobile){
            $('body').addClass('noscroll').append('<div class="previewer"><div><iframe src="//player.vimeo.com/video/79295959?title=0&amp;byline=0&amp;portrait=0&amp;color=ffffff&amp;autoplay=1" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe><p style="color:#ddd;margin-top:5px"><span class="icon icon-music" style="margin-right:10px;"></span> Soundtrack: <a href="https://www.facebook.com/UnePipeBand" target="_blank" style="color:#CD3A3D;font-weight:bold;">Une Pipe</a></p></div><span class="close icon icon-plus-sign"></span></div>');
    ...
    if( $('#landing_video').length)        
    {
        setInterval(function () { 
            ...
            if ((isActive)&&(!paused)){
                $('#landing_video').get(0).play();	
            } else {
                $('#landing_video').get(0).pause();
            }
        }, 500);    
    }
